...
namespace ContainelDll
{
    public class Container
    {
        ...
        public event EventHandler<DataEventArgs> DataEvent; //socket接收箱号数据事件
        #endregion        
        ...
        private DataEventArgs DataArgs = new DataEventArgs();
        #endregion
...
#pragma warning restore IDE0044 // 添加只读修饰符
        ...
        private void ReceiveCallBack(IAsyncResult ar)
        {
            try
            {
                ...
                string str = System.Text.Encoding.GetEncoding("GBK").GetString(buffer, 0, DataSize).Trim();
                ...
                while (str.Length > 10)//循环处理所有接收到的数据数据
                {
                    if (str.StartsWith("[C") || str.StartsWith("[U") || str.StartsWith("[N"))//判断 【箱号|重车牌|空车牌】 结果
                    {
                        ...
                        if (DataEvent != null)
                        {
                            DataArgs.Data = tmpData;
                            DataEvent(this, DataArgs);                            
                        }
